...
*/
function mobCasting(spell,mob){
    let level = mob.level
        // 创建施法环境
        let hand = InteractionHand.MAIN_HAND;
        let env = new MobCastEnv(mob, hand);
        if(mob.getAttributeValue(MieHexAttributes.MOB_MEDIA)==0){
        mob.getAttribute(MieHexAttributes.MOB_MEDIA).setBaseValue(100000);
        }
        // 创建空虚拟机
...
function allaycasting(spells,mob,media){
    ...
    mob.getAttribute(MieHexAttributes.MOB_MEDIA).setBaseValue(mob.getAttributeValue(MieHexAttributes.MOB_MEDIA)+media)
        // 创建施法环境
        let hand = InteractionHand.MAIN_HAND;
        let env = new  MobCastEnv(mob, hand);
        ...
        // 执行
        vm.queueExecuteAndWrapIotas(spells, env.world);
